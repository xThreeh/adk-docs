...
All `run_live()` code requires async context. See [Part 1: FastAPI Application Example](https://adk.dev/live/dev-guide/part1/#fastapi-application-example) for details and production examples.
...
1. **Error Recovery**: ADK supports transparent session resumption; enable via `RunConfig.session_resumption` to handle transient failures. See [Part 4: Live API Session Resumption](https://adk.dev/live/dev-guide/part4/#live-api-session-resumption) for details.
...
For session resumption and connection recovery details, see [Part 4: Live API Session Resumption](https://adk.dev/live/dev-guide/part4/#live-api-session-resumption). For multi-agent workflows, see [Best Practices for Multi-Agent Workflows](#best-practices-for-multi-agent-workflows).
...
To save audio conversations to the ADK `Session` for review or resumption, enable `RunConfig.save_live_blob = True`. This persists audio streams to artifacts. See [Part 4: save_live_blob](https://adk.dev/live/dev-guide/part4/#save_live_blob) for configuration details.
...
- **`response_modalities` controls how the model generates output**—you must choose either `["TEXT"]` for text responses or `["AUDIO"]` for audio responses per session. You cannot use both modalities simultaneously. See [Part 4: Response Modalities](https://adk.dev/live/dev-guide/part4/#response-modalities) for configuration details.
- For comprehensive coverage of audio formats, sending/receiving audio, and audio processing flow, see [Part 5: How to Use Audio, Image and Video](https://adk.dev/live/dev-guide/part5/index.md).
...
For details on enabling transcription in `RunConfig` and understanding transcription delivery, see [Part 5: Audio Transcription](https://adk.dev/live/dev-guide/part5/#audio-transcription).
...
← [Previous: Part 2: Sending Messages with LiveRequestQueue](https://adk.dev/live/dev-guide/part2/index.md) | [Next: Part 4: Understanding RunConfig](https://adk.dev/live/dev-guide/part4/index.md) →
