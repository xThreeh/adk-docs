...
For detailed information about Native Audio models and these features, see [Part 5: Audio and Video - Proactivity and Affective Dialog](https://adk.dev/live/dev-guide/part5/#proactivity-and-affective-dialog).
...
| **Session Duration**    | Audio-only: 15 min Audio+video: 2 min With [Part 4: Context Window Compression](https://adk.dev/live/dev-guide/part4/#live-api-context-window-compression): Unlimited | Both: 10 min With [Part 4: Context Window Compression](https://adk.dev/live/dev-guide/part4/#live-api-context-window-compression): Unlimited |
...
| **Tool Execution**                   | ❌ Manual tool execution and response handling   | ✅ Automatic tool execution (see [Part 3: Tool Call Events](https://adk.dev/live/dev-guide/part3/#tool-call-events))                                                                                                     |
| **Connection Management**            | ❌ Manual reconnection and session resumption    | ✅ Automatic reconnection and session resumption (see [Part 4: Live API Session Resumption](https://adk.dev/live/dev-guide/part4/#live-api-session-resumption))                                                          |
| **Event Model**                      | ❌ Custom event structures and serialization     | ✅ Unified event model with metadata (see [Part 3: Event Handling](https://adk.dev/live/dev-guide/part3/index.md))                                                                                                       |
| **Async Event Processing Framework** | ❌ Manual async coordination and stream handling | ✅ `LiveRequestQueue`, `run_live()` async generator, automatic bidirectional flow coordination (see [Part 2](https://adk.dev/live/dev-guide/part2/index.md) and [Part 3](https://adk.dev/live/dev-guide/part3/index.md)) |
...
- ADK Gemini Live API Toolkit initialization:
  ...
  - Create a [RunConfig](https://adk.dev/live/dev-guide/part4/index.md) for configuring ADK Gemini Live API Toolkit
  - Create a [LiveRequestQueue](https://adk.dev/live/dev-guide/part2/index.md) for sending user messages to the `Agent`
  - Start a [run_live()](https://adk.dev/live/dev-guide/part3/index.md) event loop
...
For the latest supported models and their capabilities, see [Part 5: Understanding Audio Model Architectures](https://adk.dev/live/dev-guide/part5/#understanding-audio-model-architectures).
...
For a detailed comparison with sequence diagrams, see [Part 4: ADK `Session` vs Live API session](https://adk.dev/live/dev-guide/part4/#adk-session-vs-live-api-session).
...
[RunConfig](https://adk.dev/live/dev-guide/part4/index.md) defines the streaming behavior for this specific session—which modalities to use (text or audio), whether to enable transcription, voice activity detection, proactivity, and other advanced features.
...
`RunConfig` is **session-specific**—each streaming session can have different configuration. For example, one user might prefer text-only responses while another uses voice mode. See [Part 4: Understanding RunConfig](https://adk.dev/live/dev-guide/part4/index.md) for complete configuration options.
...
See [Part 2: Sending messages with LiveRequestQueue](https://adk.dev/live/dev-guide/part2/index.md) for detailed API documentation.
...
See [Part 3: Event handling with run_live()](https://adk.dev/live/dev-guide/part3/index.md) for comprehensive event handling patterns.
...
- **Error handling (ADK)**: Add proper error handling for ADK streaming events. For details on error event handling, see [Part 3: Error Events](https://adk.dev/live/dev-guide/part3/#error-events).
...
- **Rate limiting and quotas**: Add rate limiting and timeout controls. For guidance on concurrent sessions and quota management, see [Part 4: Concurrent Live API Sessions and Quota Management](https://adk.dev/live/dev-guide/part4/#concurrent-live-api-sessions-and-quota-management).
...
[Next: Part 2: Sending Messages with LiveRequestQueue](https://adk.dev/live/dev-guide/part2/index.md) →
